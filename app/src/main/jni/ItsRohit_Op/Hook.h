...
// Store the start time when match begins
static std::chrono::steady_clock::time_point matchStart;
static bool timerStarted = false;

if (!current_Match) {
    timerStarted = false; // reset if match ends
} 

if (current_Match && local_player) { // ✅ only works if local_player exists
    if (!timerStarted) {
    ...
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - matchStart).count();
    ...
    ImVec2 startPos = ImVec2(screenWidth / 2, 0);  

    // End at enemy head (thoda upar)
    ...
    if (get_IsDieing(closestEnemy)) {
        ...
        // Alive enemy = 🟢 Green line
        draw->AddLine(startPos, endPos, ImColor(0, 255, 0), 1.7f);
    }
}
...
if (Config.ESP.Box) {
    ...
    if (get_IsDieing(closestEnemy)) {
        ...
        ImColor color = ImColor(0, 255, 0); // 🟢 Green
